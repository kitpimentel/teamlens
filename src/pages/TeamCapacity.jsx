...
import OptimizeAllocationModal from "@/components/team/OptimizeAllocationModal";
...
export default function TeamCapacity() {
  ...
  const [periodFilter, setPeriodFilter] = useState("Q2 2024");

  // Team capacity metrics with state management
  ...
  // Handle applying optimization changes
  const handleApplyOptimizationChanges = (optimizedTeam) => {
    setTeamMembers(optimizedTeam);
    
    // Recalculate capacity metrics based on new allocations
    // This is a simplified calculation
    const avgAvailability = optimizedTeam.reduce((sum, member) => sum + member.availability, 0) / optimizedTeam.length;
    
    // Update the metrics
    const frontendDevs = optimizedTeam.filter(m => m.role.includes("Frontend"));
    const backendDevs = optimizedTeam.filter(m => m.role.includes("Backend"));
    const designers = optimizedTeam.filter(m => m.role.includes("Designer") || m.role.includes("UX"));
    const managers = optimizedTeam.filter(m => m.role.includes("Manager"));
    
    // Calculate new department metrics
    const newFrontendCapacity = frontendDevs.length > 0 
      ? Math.round(frontendDevs.reduce((sum, dev) => sum + dev.availability, 0) / frontendDevs.length)
      : capacityMetrics.frontend;
      
    const newBackendCapacity = backendDevs.length > 0 
      ? Math.round(backendDevs.reduce((sum, dev) => sum + dev.availability, 0) / backendDevs.length)
      : capacityMetrics.backend;
      
    const newDesignCapacity = designers.length > 0 
      ? Math.round(designers.reduce((sum, des) => sum + des.availability, 0) / designers.length)
      : capacityMetrics.design;
      
    const newManagementCapacity = managers.length > 0 
      ? Math.round(managers.reduce((sum, mgr) => sum + mgr.availability, 0) / managers.length)
      : capacityMetrics.management;
      
    // Update the capacity metrics state
    setCapacityMetrics({
      overall: Math.round(avgAvailability),
      frontend: newFrontendCapacity,
      backend: newBackendCapacity,
      design: newDesignCapacity,
      management: newManagementCapacity
    });
  };
  ...
  return (
    <div className="flex h-screen bg-gray-50">
      ...
      <div className="flex-1 flex flex-col overflow-hidden">
        <Header username="Sergey Goldberg" currentPage="Team Capacity" />
          ...
          <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-6 gap-4">
            ...
            <div className="flex items-center space-x-3">
              ...
              <Button 
                ...
                onClick={() => {
                  // This would normally open a dropdown or date picker
                  // For demo purposes, we're just cycling through quarters
                ...
                <div className="space-y-3">
                  <div className="flex justify-between items-center p-2 bg-green-50 rounded-lg">
                    ...
                    <span className="text-sm font-bold">
                      {teamMembers.filter(m => m.availability >= 70).length} team members
                    </span>
                  </div>
                  ...
                  <div className="flex justify-between items-center p-2 bg-amber-50 rounded-lg">
                    ...
                    <span className="text-sm font-bold">
                      {teamMembers.filter(m => m.availability >= 40 && m.availability < 70).length} team member
                    </span>
                  </div>
                  ...
                  <div className="flex justify-between items-center p-2 bg-red-50 rounded-lg">
                    ...
                    <span className="text-sm font-bold">
                      {teamMembers.filter(m => m.availability < 40).length} team member
                    </span>
                  </div>
